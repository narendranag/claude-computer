...
3. Run `~/system-manager/bin/archive-push <paths> --dry-run`, then — **after I say yes** — `archive-push <paths> --delete-local --yes`. The script asks before each deletion and answers itself no when there is no terminal, so `--yes` is what carries my answer through; never pass it before I have given it.
