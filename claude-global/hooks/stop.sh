...
CC="${CC_HOME:-$HOME/claude-computer}"
...
if [ -d "$CC/.git" ] && [ ! -f "$CC/.template" ] && [ -n "$(git -C "$CC" status --porcelain -- docs 2>/dev/null)" ]; then
  files="$(git -C "$CC" status --porcelain -- docs | awk '{print $NF}' | sed 's|^docs/||' | head -5 | paste -sd ', ' -)"
  git -C "$CC" add -- docs
  if out="$(git -C "$CC" commit -q -m "[$host] update $files" -- docs 2>&1)"; then
    # Machine files are private: never push them to a public repository.
    vis="$(cd "$CC" && to 15 gh repo view --json visibility --jq .visibility 2>/dev/null)"
    if [ "$vis" = "PUBLIC" ]; then
      echo "claude-computer: committed docs locally but NOT pushed — origin is a public repo" >&2
    else
      to 30 git -C "$CC" push -q 2>/dev/null || echo "claude-computer: committed docs but push failed (offline?)" >&2
    fi
  else
...
if [ -f "$start_file" ]; then
  ...
  if [ "$elapsed" -ge $(( ${CC_LONG_TURN_MIN:-10} * 60 )) ]; then
    "$CC/bin/tg-send" "done after $((elapsed / 60)) min in ${cwd/#$HOME/~}" >/dev/null 2>&1 || true
  fi
fi
exit 0
