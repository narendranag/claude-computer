...
[ -d "$CC/.git" ] || { echo "claude-computer: no repo at $CC — run /setup"; exit 0; }

if out="$(to 20 git -C "$CC" pull --rebase --autostash -q 2>&1)"; then
  echo "claude-computer: pulled ($(git -C "$CC" log -1 --format='%h %s' | cut -c1-72))"
else
  echo "claude-computer: pull FAILED — working from local copy. Resolve before editing docs/. ${out:0:200}"
fi

case "$("$CC/bin/secrets-unlock" --status 2>/dev/null)" in
...
if [ -d "$HOME/vault" ]; then
  ...
  [ -f "$HOME/Library/Application Support/MacParakeet/macparakeet.db" ] && to 30 "$CC/bin/transcripts-sync" 2>&1 | head -1
  to 30 "$CC/bin/tasks-sync" 2>&1 | head -1
fi
exit 0
